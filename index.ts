...
console.log(1,process.env['X-RapidAPI-Key']);
...
interface Options {
    method: 'GET' | 'POST' | 'PUT' | 'DELETE'; // Set the HTTP method to a valid value
    headers: {
      'X-RapidAPI-Key': any;
      'X-RapidAPI-Host': string;
    };
}
async function getConversionRate(currency1: string, currency2: string, amount: number) {
    const url = `https://currency-converter-by-api-ninjas.p.rapidapi.com/v1/convertcurrency?have=${currency1}&want=${currency2}&amount=${amount}`;
    ...
    if (json.new_amount) {
        return json.new_amount;
    } else {
        throw new Error(`Currency conversion failed: ${json.error}`);
    }
}
...
async function start() {
    ...
    if (!answer.amount) {
        ...
        const { currency1, currency2, amount } = answer;
        try {
            const conversionRate = await getConversionRate(currency1, currency2, amount);
            console.log(`${amount} ${currency1} is equal to  ${conversionRate} ${currency2}`);
        } catch (error) {
            console.error(error);
        }
    }
